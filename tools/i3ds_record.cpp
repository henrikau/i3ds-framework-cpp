...
void
write_msg_to_file(std::ofstream &output_file, long long timestamp, const i3ds::Message &msg)
{
  size_t msg_size = msg.size();
  output_file.write((char*)&timestamp, sizeof(long long));
  output_file.write((char*)&msg_size, sizeof(size_t));
  output_file.write((char*)msg.data(), msg_size);
}
...
main(int argc, char *argv[])
{
  ...
  long long delay; 
  long long start_time = get_current_time_in_us();
  BOOST_LOG_TRIVIAL(trace) << "Started at time: " << start_time;
  unsigned int messages_received = 0;
  while(running)
    {
      subscriber->Receive(msg);
      delay = get_current_time_in_us() - start_time;
