...
run_test(std::shared_ptr<typename Codec::Data> data, unsigned int n_replications, unsigned int floats_per_measurement)
{
  ...
  std::cout << "  Total time:  " << total_times
            << " us/f: " << std::setw(9) << static_cast<double>(total_times.mean())/floats_per_measurement << std::endl <<
            std::endl;
}
...
void
camera_test(unsigned int n_replications, unsigned int size_x, unsigned int size_y, unsigned int pixel_size,
